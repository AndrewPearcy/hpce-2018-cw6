...
if [[ $VIS == 1 ]]; then
    ...
    (
    # Convert the objects to spikes and classify it
    bin/frame_to_spikes < ${WORKING}/objects.mjpeg | 
    # Then back into frames
    bin/spikes_to_frame | 
    # And then do the detection
    bin/frame_to_object_stream > ${BASE}.spiking.events
    )&
    (
    # Sub-sample the objects to frames and classify it
    bin/frame_subsample ${OVERSAMPLE} < ${WORKING}/objects.mjpeg | 
    # ... then do the detection
    bin/frame_to_object_stream |
    # ... and then stretch time to reflect that this stream is much slower than the original
    bin/object_stream_stretch ${OVERSAMPLE} > ${BASE}.trad.events
    )
    wait
